...
## Попытка установить SSL сертификат на поддомен через панель
Ошибка. Говорит измените записи А чтобы были одинаковые IP для домена и поддомена. Что невозможно. Ответ службы поддержки Бегет:
Здравствуйте!

К сожалению, установить SSL-сертификат от Let’s Encrypt на VPS-сервер через нашу панель управления не получится - он находится полностью под вашим администрированием.

Прикладываю инструкции по установке SSL-сертификатов от Let’s Encrypt для различных панелей управления:

Статья для VestaCP

Статья для HestiaCP или видео-инструкция

Статья для ISPmanager или видео-инструкция

В случае если используется веб-сервер Apache или Nginx:

Видео-инструкция для Apache

Видео-инструкция для Nginx
